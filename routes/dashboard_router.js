...
router.post("/check-user", async (req, res) => {
    // console.log(client.db("user").collection("user"))
    // client.db("user").collection("user").insertOne({
    //     username: "admin",
    //     password: "12345678", // You should hash the password before inserting it
    //     // other user fields here
    //   })
    let result = await client.db("user").collection("user").find({ username: req.body.username, password: req.body.password }).toArray();
    console.log(result)
    if (result.length != 0) {
