                        ...
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item d-md-flex d-grid">
                                <div class="flex-even">
                                    ...
                                    <span id="q-${element._id}">${element.current_stock
						} <span class="stat">${element.updated_stock > 0
							? `(+${element.updated_stock})`
							: element.updated_stock < 0
								? `(${element.updated_stock})`
								: ""
						}</span></span>
                                </div>
                                ...
                                <div class="flex-even">
                                    <span>PP/Item:</span>
                                    <span>₦ ${element.purchase_price}</span>
                                </div>
                                <div class="flex-even">
                                    <span>SP/Item:</span>
                                    <span>₦ ${element.selling_price}</span>
                                </div>
                                <div class="flex-fill icons-container">
                                    <button class="material-icons" onclick="edit('${element.name
						}')">edit_note</button>
                                    <div class="btn-box remove">
                                        <input type="text" id="rem-${element._id
						}" maxlength="5">
                                        <button class="material-icons" onclick="update('${element._id
						}',false)">remove_circle</button>
                                    </div>
                                    <div class="btn-box add">
                                        <input type="text" id="add-${element._id
						}" maxlength="5">
                                        <button class="material-icons" onclick="update('${element._id
						}',true)">add_circle</button>
                                    </div>
                                    <span class="material-icons delete" onclick="deleteStock('${element._id
						}')">delete</span>
                                </div>
                            </li>
                        </ul>
		...
		.then(async function (res) {
			if (res.status) {
				...
				res.items.forEach((element, index) => {
					const thisTotalSale = element.sold_stock * element.selling_price;
				...
				$("#pdf").prepend(`
                    <b>Total Sale: ₦ ${totalSale}</b><br>
                    <b>Total Profit: ₦ ${totalProfit}</b>
                `);
